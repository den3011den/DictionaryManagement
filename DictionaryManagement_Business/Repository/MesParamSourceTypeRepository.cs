...
namespace DictionaryManagement_Business.Repository
{
    public class MesParamSourceTypeRepository : IMesParamSourceTypeRepository
    {
        ...
        {
            var objToGet = await _db.MesParamSourceType.FirstOrDefaultAsync(u => u.Id == Id);
            if (objToGet != null)
            {
        ...
        public async Task<IEnumerable<MesParamSourceTypeDTO>> GetAll(SelectDictionaryScope selectDictionaryScope = SelectDictionaryScope.All)
        {
            ...
            {                
                return _mapper.Map<IEnumerable<MesParamSourceType>, IEnumerable<MesParamSourceTypeDTO>>(_db.MesParamSourceType);
            }
            if (selectDictionaryScope == SD.SelectDictionaryScope.ArchiveOnly)
                return _mapper.Map<IEnumerable<MesParamSourceType>, IEnumerable<MesParamSourceTypeDTO>>(_db.MesParamSourceType.Where(u => u.IsArchive == true));
        ...
        {
            var objectToUpdate = _db.MesParamSourceType.FirstOrDefault(u => u.Id == objectToUpdateDTO.Id);
        ...
        {
            var objToGet = await _db.MesParamSourceType.FirstOrDefaultAsync(u => u.Name.Trim().ToUpper() == name.Trim().ToUpper());
            if (objToGet != null)
            {
                return _mapper.Map<MesParamSourceType, MesParamSourceTypeDTO>(objToGet);
            }
